		...
		methods: {
			...
			async loadCommunity() {
				...
				try {
					...
					if (resp.code === 1) {
						const villageName = resp.data.villagename
						// 如果之前访问过小区，直接展示信息
			...
			async handleSubmit() {
				...
				if (this.visitorType === 'delivery') {
					...
					try {
						...
						// 调用提交API
						const res = await this.$api.visitorRegister(submitData);
						if (res.code === 1) {
							uni.showToast({
								title: '登记成功',
								icon: 'success'
				...
				if (this.visitorType === 'normal') {
					...
					try {
						...
						const submitData = {
							...
							owner_tel: this.formData.phone,
							visit_time: this.getCurrTime()
						};

						// 调用提交API
						const res = await this.$api.visitorHistorySave(submitData);
						if (res.code === 1) {
							uni.showToast({
								title: '登记成功',
								icon: 'success'
