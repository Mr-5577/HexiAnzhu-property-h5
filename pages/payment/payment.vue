	...
	<view class="uni-padding-wrap">
		...
		<view class="type-select">
			<view class="payment-content" v-if="type == '1'">
				<view class="payment-content-cell" style="justify-content: space-between;"
					v-if="payData.wechatPay || payData.aliPay" @click="toBalance">
					<template>
						<view class="payment-content-cell" style="border: none;">
							...
							<view class="uni-cell-lineheight-80"
								:class="{ 'uni-text-gray': payData.balance < orderInfo.money }" :loading="item.loading">
								当前余额 ： {{payData.balance}}</view>
							<view class="uni-cell-lineheight-80 uni-text-gray" v-if="payData.balance < orderInfo.money">
								(余额不足)
							</view>
						</view>
						<view class="uni-cell-lineheight-80"
							v-if="payData.balance > orderInfo.money || payData.balance == orderInfo.money "
							style="margin: 10upx;">
							余额支付
						</view>
					</template>
			...
			<view class="payment-content">
				<view class="payment-content-cell" :class="{'po-event-none' : disabled == true}"
					v-for="(item, index) in providerList"
					v-if="(item.id == 'wxpay' &&  payData.wechatPay == true) || (item.id == 'alipay' &&  payData.aliPay == true)"
					:key="index" @click="requestPayment(item, index)">
