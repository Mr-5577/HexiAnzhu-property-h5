			...
			<view class="uni-flex-center qf-wrapper">
				<view class="uni-cell-94 est-content">
					...
					<view class="uni-flex-btw est-content-header">
						<checkbox style="transform:scale(0.7);visibility:hidden"></checkbox>
...
export default {
	...
	computed: {
		...
		// 预缴金额（优惠后）
		totalPrepaidAmount() {
			if (!this.preCarData || !this.preCarData.fee || this.monthNum <= 0) {
				return 0;
			}
			const fee = Number(this.preCarData.fee);
		...
		totalDiscountAmount() {
			let total = 0;
			// 计算欠费部分的优惠金额
			this.carDetailList.forEach(item => {
				if (item.checked) {
					// 欠费项目的优惠金额
		...
		discountDescription() {
			...
			if (this.monthNum > 0 && this.preCarData && this.preCarData.yjmonth) {
				const prepaidDiscount = parseFloat(this.totalDiscount) || 0;
				if (prepaidDiscount > 0) {
					...
					if (this.selectedGroup) {
						...
						// 没有选中方案时显示总优惠
						// const startMonth = this.preCarData.yjmonth;
	...
	methods: {
		...
		// 年月递增 处理数据
		convertMonthlyData() {
			const { yjmonth, fee } = this.preCarData;
			if(!(yjmonth && fee)) {
				return []
			}
			const result = [];
		...
		checkBoxs(w) {
			...
			this.$forceUpdate();
			// 取消优惠金额。置空优惠方案
			this.carDetailList.map((item) => item.disc_fee = 0)
			this.totalDiscount = 0
			this.clearSelection()
		},
		activeRule(){
		...
		async payment() {
			...
			const newCostList = preCarList.concat(costList)
			const params = {
				resources_type: 'car',
