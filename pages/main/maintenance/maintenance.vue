<template>
	<view class="maintenance">
		<view class="content">
						...
						<view v-for="(item, index) in items" :key="index">
							<label :style="{ color: index == delCurrent ? '#ffcf5a' : '#000000' }"
								...
								<radio color="#ffcf5a" :value="item.value" :checked="index === delCurrent"
									style="transform: scale(0.7);margin-left: -5upx;" />
								{{ item.text }}
							</label>
						</view>
					</radio-group>
					...
					<scroll-view class="scroll-view_H" scroll-x="true">
						<radio-group style="display: flex;">
							<view style="display: inline-block;" @click="radioChangePerson(index)"
								:class="['person', index === personCurrent ? 'active' : '']"
			...
			<w-picker mode="limit" dayStep="60" startHour="8" endHour="20" minuteStep="5" afterStep="30"
				:defaultVal="[0, 0, 0]" :current="true" @confirm="onConfirm" ref="limit"
				themeColor="#ffcf5a"></w-picker>
	...
	export default {
		...
		data() {
			return {
				...
				current: 12, //默认报事报修12,业主投诉34
				baoshi: '',
				jianyi: '',
				type1: [
					{ id: 1, typename: "房屋维修", ptype: 1 },
					{ id: 2, typename: "水电燃气", ptype: 1 },
					{ id: 3, typename: "公共设施", ptype: 1 },
					{ id: 4, typename: "其他类型", ptype: 1 },
				], //报事类型
				type2: [
					{ id: 6, typename: "绿化管理类", ptype: 2 },
					{ id: 7, typename: "设施设备类", ptype: 2 },
					{ id: 8, typename: "秩序管理类", ptype: 2 },
					{ id: 9, typename: "清洁管理类", ptype: 2 },
					{ id: 10, typename: "其他类型", ptype: 2 },
					{ id: 11, typename: "客户服务类", ptype: 2 },
					{ id: 12, typename: "地产相关类", ptype: 2 },
				], //投诉类型
				items: [{
					text: '立即处理',
					value: '1'
		...
		methods: {
			...
			async sub() {
				if (!this.$store.state.login_token) {
					uni.showModal({
						title: '提示',
						content: '此功能需要验证您的身份，登录后可处理报修事宜。是否前往登录？',
						cancelColor: '#898989',
						cancelText: '取消',
						confirmColor: '#fe845e',
						confirmText: '去登录',
