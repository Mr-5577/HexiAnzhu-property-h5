					...
					<view class="uni-flex-center">
						<view class="uni-cell-100">
							...
							<view class="report-content">
								<view v-for="(item, index) in type1" :key="index" @click="changeType(index,1)" class="_box" :class="{ _boxActive: indexType === index }">
									{{ item.typename }}
								</view>
							</view>
						</view>
		...
		imageUploads() {
			...
			return new Promise(resolve => {
				...
				_this.complaintImages.forEach(filePath => {
					let key = bucket + '-' + this.nowDay + '-' + new Date().getTime() + '.jpg';
					keys.push(key);
