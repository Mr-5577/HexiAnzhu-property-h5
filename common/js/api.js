...
// 模拟登录接口
apis.mockLogin = (data) => {
	return uniRequest.post("/apiNew/mockLogin", data, config);
};
// 获取活动列表
 ...
 * @name 添加发票抬头
 * @param {Object} data
 * @param {string} data.inv_title - 抬头名称
 * @param {number} data.union_code - 统一社会信用代码 / 身份证号
 * @param {number} data.inv_type - 抬头类型：1=个人，2=企业
 * @param {string} data.inv_special - 是否默认专票：0/1（仅企业可用，个人传 1 会被拒）
 * @param {string} data.email - 收票邮箱
 * @param {string} data.tel - 联系电话
 * @param {string} data.default - 是否设为默认：0/1，置 1 会清除其他默认
 */
apis.invoiceTitleAdd = (data) => {
	return uniRequest.post("/apiNew/invoiceTitleAdd", data, config);
};
/**
 * @name 修改发票抬头
 * @param {Object} data
 ...
 * @param {string} data.default - 是否设为默认：0/1，置 1 会清除其他默认
 */
apis.invoiceTitleEdit = (data) => {
	return uniRequest.post("/apiNew/invoiceTitleEdit", data, config);
};
/**
 * @name 删除发票抬头
 * @param {Object} data
 * @param {number} data.id - ID
 */
apis.invoiceTitleDel = (data) => {
	return uniRequest.post("/apiNew/invoiceTitleDel", data);
};
/**
 * @name 发票抬头详情
 * @param {Object} data
 * @param {number} data.id - ID
 */
apis.invoiceTitleDetail = (data) => {
	return uniRequest.post("/apiNew/invoiceTitleDetail", data);
};
/**
 * @name 发票抬头列表
 * @param {Object} data
 ...
 */
apis.invoiceTitleList = (data) => {
	return uniRequest.post("/apiNew/invoiceTitleList", data);
};
/**
 * @name 可开票列表
 * @param {Object} data
 ...
 * @param {number} data.limit - 每页条数
 */
apis.invoiceCanList = (data) => {
	return uniRequest.post("/apiNew/invoiceCanList", data, config);
};
/**
 * @name 已开票列表
 * @param {Object} data
 ...
 * @param {number} data.limit - 每页条数
 */
apis.invoiceMadeList = (data) => {
	return uniRequest.post("/apiNew/invoiceMadeList", data, config);
};
/**
 * @name 批量开票
 * @param {Object} data
 * @param {Array} data.ids - 订单 ygzy_sn.id 列表，支持逗号分隔字符串 "88,89" 或数组
 * @param {Array} data.title_id - 抬头 ID（须属于当前业主且未删除）
 * @param {number} data.fp_type - 发票类型：1=普票（默认），2=专票
 */
apis.invoiceBatch = (data) => {
	return uniRequest.post("/apiNew/invoiceBatch", data, config);
};
